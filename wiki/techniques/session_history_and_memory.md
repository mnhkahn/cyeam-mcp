...
sources: ["59a85f143646"]
original_filename: 2026-06-02-session-history-and-memory.md
description: 深入解析 Claude Code 的两套数据持久化机制——会话历史（Session History）和记忆（Memory），以及 Archivist、Dream 等核心组件如何协同工作。
---

# Claude Code 的会话历史与记忆系统

深入解析 Claude Code 的两套数据持久化机制——会话历史（Session History）和记忆（Memory），以及 Archivist、Dream 等核心组件如何协同工作。
