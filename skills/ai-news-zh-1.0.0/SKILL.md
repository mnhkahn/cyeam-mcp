...
1. **禁止编造**：所有标题、摘要、链接必须来自工具返回的实时数据。绝不允许使用训练数据中的旧事件冒充今日新闻。
2. **数据来源唯一**：本任务的"事实"由 `curl` 工具返回的数据提供（推荐由调用方预取并传入），LLM 只做翻译/分类/排版。
3. **日期必须对齐**：`AI 早报 | YYYY.MM.DD` 头部必须用系统注入的 `真实当前日期`，不要按自己印象写。
4. **链接原样照抄**：每条新闻的 `🔗` 链接必须是工具返回的链接原值，不允许替换、缩短或"我猜这个 URL 应该差不多"。

## 数据采集命令

```bash
curl "https://www.theverge.com/ai-artificial-intelligence"
curl "https://www.wired.com/feed/tag/ai/latest/rss"
curl "https://techcrunch.com/feed/"
curl "https://www.anthropic.com/news"
curl "https://www.technologyreview.com/feed/"
curl "https://blog.google/technology/ai/"
```
